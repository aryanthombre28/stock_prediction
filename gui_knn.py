...
def gen_charts2(stock_name):
    ...
    predprices_train = knn_model.predict(x_train)
    predprices_test = knn_model.predict(x_test)

    #Making predictions for the next day (using the last sequence from the test set)
    last_sequence = x_test[-1:]
    y_pred = knn_model.predict(last_sequence)
    ...
    #r2t = np.round(r2t, 3)
    mse_train = np.round(mse_train, 5)
    mse_test = np.round(mse_test, 5)
    ...
    mse_label = tk.Label(window, text = "Mean Square Error(MSE) on test data is : "+str(mse_test))
    r2tr_label.pack()
    #r2t_label.pack()
    ...
    pred_label = tk.Label(window, text="Predicted Price (Random Forest): Rs." + str(y_pred))
    pred_label.pack()
